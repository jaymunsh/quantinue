...
### 07-17 — v3.9: 1차 MVP 동결본 분리 (태그 v3.9)

- 상황: **1차 MVP 코드가 완성 커밋됨** (`6163630` "1차 MVP 운영 파이프라인과 관리자 화면 완성" — 다른 작업자). 이에 맞춰 문서를 1차 전문으로 마무리.
- **동결본** = `docs/quantinue-integrated-design-mvp1.html` — v3.8 그대로 복사 + 최상단 동결 배너(남색) + 히어로 배지 "🧊 동결본(수정 금지)" + title 변경. **이후 절대 수정하지 않음** — 1차 결정 과정(#1~#23 카드·협의 현황판·Q&A) 보존, 트러블슈팅·회고 참조용.
- **정본** = `quantinue-integrated-design.html` **이름 유지** (링크·git 이력·changelog 연속성 보존) — 히어로에 동결본 링크 추가.
- **다음 계획 (사용자와 합의)**: 2차 최종 설계는 정본을 **v4.0으로 대개편** — ① 협의·수정 필요 항목을 먼저 대화로 확정(새 문서 초반에 "2차 확정 사항"으로 명기) ② superpowers 등 계획 스킬 활용해 계획 수립 후 진행 ③ 이력성 내용(카드·해소된 협의) 제거하되 **컨벤션 8규칙·점수 눈금·뒤집기 트리거(한 줄 결정 로그 표로 압축)·열린 안건은 이월** ④ 디자인 전면 재설계(독자 작업 중심 구조) ⑤ 이후 에이전트 개발용 md 추출.
