...
```bash
docker compose --project-name quantinue_mvp up --build --wait
```

Docker 실행 화면은 `http://127.0.0.1:8011`, DB는
...
```bash
docker compose --project-name quantinue_mvp down
```
...
|---|---|---|---|
| `role_01_universe_screener` | 1차 스크리너 | fixture 또는 공개 NASDAQ 유니버스 | 공급자 정책 고도화 |
| `role_02_technical_analysis` | 기술 분석 | fixture 또는 공개 일봉 기반 지표 | 지표·조정주가 고도화 |
| `role_03_daily_screener` | 2차 스크리너 | 점수 게이트 | 5버킷 오늘의 50 |
...
| `role_06_news_analysis` | 뉴스 분석 | fixture 또는 공개 RSS 제목·스니펫 분석 | 종목별 RSS 고도화 |
| `role_07_strategist` | 전략 종합 | 코드 게이트와 buy | 정책·프롬프트 |
