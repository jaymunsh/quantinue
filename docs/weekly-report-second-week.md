...
남은 핵심 개발은 하나다. **장중에 새로 들어온 공시·뉴스만 수집하고, 관련 종목만 다시 판단해 필요한 경우 모의 매매로 연결하는 사건 경로**를 완성해야 한다.

### 7.1 지금 바로 진행할 개발

| 순서 | 개발 항목 | 완료 조건 |
|---:|---|---|
| 1 | 사건 원장 | 공시·기사 원문, 수집 위치, 사건, 처리 결과를 덮어쓰지 않고 저장 |
| 2 | 장중 증분 수집 | SEC 공시 30분, 일반 뉴스 15분, 와이어 10분 주기로 새 자료만 수집 |
| 3 | 코드 필터 | 중복·중요도·관련 종목을 먼저 판정해 불필요한 AI 호출 차단 |
| 4 | 근거 묶음 | 짧은 원문은 직접 전달하고 12,000자를 넘는 문서만 요약해 재사용 |
| 5 | 관련 종목 재판단 | 보유 종목과 당일 후보 중 사건과 관련된 종목만 전략가·독립 비평가에게 전달 |
| 6 | 자동 모의 집행 | 바뀐 판단이 매수면 배분·매수, 매도면 청산, 보류면 주문 없이 유지 |

1~6번은 AWS 없이 현재 로컬 환경에서 개발할 수 있다.

### 7.2 개발이 끝난 뒤 확인할 것

- 새 사건이 없으면 AI 호출이 0회인지 확인한다.
- 같은 사건을 다시 받아도 재처리와 추가 주문이 0회인지 확인한다.
- 관련 사건 하나가 들어오면 해당 종목만 정확히 1회 판단하는지 확인한다.
- 뉴욕 정규장 1~2거래일 동안 중복 판단·주문 없이 이어지는지 관측한다.

### 7.3 마지막에 결정할 것

AWS 이전은 개발 항목이 아니다. 로컬 운영 증거가 충분히 쌓인 뒤 상시 가동이 필요할 때만 결정한다.
