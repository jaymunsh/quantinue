...
4. **새 공시·뉴스에 따른 장중 AI 재판단은 아직 개발 중이다.** 현재 공시와 뉴스는 일일 전체 체인에서만 수집한다.
...
아래 표에서 가격 1분 감시는 현재 동작 중이다. 공시·뉴스·와이어의 장중 반복 수집은 아직 동작하지 않는다.
...
일일 전체 체인은 자동화됐고 장중 가격 감시 기반도 구현됐다. 다만 실제 정규장에서의 연속 관측과 장중 신규 공시·뉴스 사건 경로는 아직 남아 있다. 다음 주에는 공시·뉴스 사건 경로를 먼저 개발하고, 이후 장중 기능 전체를 정규장에서 검증해 “코드가 존재하는 상태”에서 “사람의 반복 조작 없이 안전하게 운영되는 상태”로 마무리한다.
