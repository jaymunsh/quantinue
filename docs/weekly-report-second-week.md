...
2. **뉴욕 정규장에는 가격을 1분마다 확인한다.** 현재 장중에 실제로 활성화된 매매는 손절·익절 조건에 따른 방어 매도다.
3. **가격 변화에 따른 장중 AI 매수·매도·보류는 코드까지 구현됐지만 꺼져 있다.** 정규장 검증을 통과한 뒤 단계적으로 활성화한다.
4. **새 공시·뉴스에 따른 장중 AI 재판단도 구현됐다.** SEC 30분·일반 뉴스 15분·와이어 10분의 증분 경로이며, 현재는 실제 정규장 관측 전이라 `rejudge=false`로 꺼 두었다.
...
아래 주기는 코드와 설정에 구현돼 있다. 가격 1분 감시는 poll-only 실장 확인
중이고, 공시·뉴스·와이어의 유료 재판단은 `rejudge=false`라 아직 동작하지
않는다.
