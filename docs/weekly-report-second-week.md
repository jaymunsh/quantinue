...
| 항목 | 내용 |
|---|---|
| 보고 기간 | 2026-07-18 ~ 2026-07-24 |
| 대상 | Quantinue MVP-2 |
| 팀 | 여름이었다 |
| 한 줄 결론 | **하루 한 번 실행되던 시연형 파이프라인을 장애 탐지와 장중 관측이 가능한 모의투자 운영 시스템으로 확장했다.** |
...
1. **거래일마다 일일 전체 체인이 한 번 자동 실행된다.** 데이터를 수집하고 두 전략가와 독립 비평가가 매수·매도·보류를 판단한 뒤 MockBroker에서 모의 주문한다.
2. **뉴욕 정규장에는 가격을 1분마다 확인한다.** 7월 24일 이 PC의 8020에 poll-only 운영을 반영했으며, 현재 장중에 활성화된 매매는 손절·익절 조건에 따른 방어 매도다.
3. **가격 변화에 따른 장중 AI 매수·매도·보류는 코드까지 구현됐지만 꺼져 있다.** 정규장 검증을 통과한 뒤 단계적으로 활성화한다.
4. **새 공시·뉴스에 따른 장중 AI 재판단도 구현됐다.** SEC 30분·일반 뉴스 15분·와이어 10분의 증분 경로이며, clean OpenAI 슬롯과 Gate B 전까지 `rejudge=false`로 둔다.

AWS 이전은 이 기능들과 무관하다. 위 개발과 검증은 모두 현재 로컬 환경에서 진행할 수 있으며, AWS는 로컬 운영이 안정된 뒤 결정한다.
...
flowchart TB
    ...
    F["1분 가격 방어 · 로컬 운영 ON"] -->|손절·익절| D
    G["가격 사건 AI 재판단 · 비활성"] -.-> C
    H["공시·뉴스 사건 재판단 · 구현 완료·비활성"] -.-> C
```

일일 전체 분석은 한 번 실행한다. 대신 장중에는 모든 과정을 반복하지 않고, 가격 변화나 새 공시·뉴스처럼 **새로운 사건이 생겼을 때 관련 종목만 다시 판단**하는 경로를 별도로 구현했다. 실제 운영 활성화는 정규장 검증 뒤에 한다.
...
이 판단과 후속 모의 집행 경로는 가격 사건과 공시·뉴스 사건 모두 구현돼 있지만 현재 운영에서는 비활성이다. 지금 활성화 후보인 장중 동작은 1분 가격 조회와 손절·익절 방어 매도다.

### 장중 사건 경로까지 완성된 뒤의 목표 주기

아래 주기는 코드와 설정에 구현돼 있다. 가격 1분 감시는 8020에서 poll-only로
운영 중이고, 공시·뉴스·와이어의 유료 재판단은 `rejudge=false`라 아직
동작하지 않는다.
...
장중 신규 공시·뉴스 사건 경로까지 구현됐으므로 큰 기능 개발은 끝났다.
이제 남은 핵심은 **이미 만든 기능을 실제 정규장에서 단계적으로 켜고,
중복 판단·주문 없이 작동한다는 운영 증거를 쌓는 것**이다.

### 7.1 구현이 끝난 항목

| 순서 | 구현 항목 | 현재 상태 |
|---:|---|---|
| 1 | 사건 원장 | 원문·버전·cursor·정규화 사건·처리 영수증 구현 완료 |
| 2 | 장중 증분 수집 | SEC 30분·일반 뉴스 15분·와이어 10분 경로 구현 완료 |
| 3 | 코드 필터 | 중복·중요도·관련 종목 선필터 구현 완료 |
| 4 | 근거 묶음 | 짧은 원문 직접 전달·긴 문서 요약 캐시 구현 완료 |
| 5 | 관련 종목 재판단 | 전략가·critic의 persona별 at-most-once 경로 구현 완료 |
| 6 | 자동 모의 집행 | 변경 판단만 기존 청산·배분 경로에 연결 완료 |

### 7.2 실제 운영에서 확인할 것

1. poll-only를 뉴욕 정규장 한 세션 동안 연속 관측한다.
2. 다음 일일 슬롯에서 8020-only 잡·LLM usage·비용·retry·알림을 대조한다.
3. `rejudge=true`, `stream=false`로 사건·정기 재판단을 켜 기능일과 다음
   거래일의 중복 호출·주문 0건을 확인한다.
4. 마지막에 보유 종목 stream을 켜 재연결과 poll fallback·롤백을 확인한다.

### 7.3 마지막에 결정할 것

Terminal 탭 종료·로그아웃·재부팅에도 자동 복구할 로컬 supervisor와 AWS
이전 여부는 기능 개발과 별개다. 로컬 운영 증거가 충분히 쌓인 뒤 상시 가동
방식을 결정한다.
...
| 장중 신규 사건 대응 지연 | 공시·뉴스·와이어 증분 경로 구현 완료, 실제 정규장에서는 Gate 순서대로 활성화 |
...
일일 전체 체인은 자동화됐고 장중 가격 감시와 신규 공시·뉴스 사건 경로도
구현됐다. 남은 필수 작업은 실제 정규장에서 poll-only 연속 관측, 깨끗한
OpenAI 일일 슬롯 대조, 사건 재판단 기능일과 다음 거래일 안정성, 마지막
웹소켓 연결·롤백이다. 다음 주에는 이 순서로 운영 증거를 쌓아 “코드가 존재하는
상태”에서 “사람의 반복 조작 없이 안전하게 운영되는 상태”로 마무리한다.
