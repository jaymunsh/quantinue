...
- TDD: 실패 테스트 → 최소 구현 → green → 태스크 단위 1커밋. 유닛/웹 **681 green** 유지(계약 변경 시 테스트 같은 커밋 갱신).
...
- `tb_daily_bar` 신설(**소비자와 같은 커밋**) + Alpaca 마켓데이터 어댑터: 배치 일봉(다중 심볼) + 현재 호가. 무료 IEX 티어 전제 — ⏳ 엔드포인트·한도 착수 시 확인. 키는 `.env`에 기존재.
...
- 새 경로가 구 러너와 E2E 동등성 통과 → **11단계 러너 삭제**.
