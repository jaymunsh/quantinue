...
> ✅ **2026-07-20 완료.** 대시보드 잡 기반 전환(`GET /`) · **구 11단계 러너 삭제** ·
> 정본 HTML 미러(v5.3, 잡 체인이 정본 자리로) · ghost 재감사(config 유령 4건 제거).
> 상세는 dev-handoff "Phase 5 완료".
...
**이중 캘린더는 완료하지 않았다 — 전제가 틀렸다.** role_11 삭제로 자연소멸할
것으로 봤지만 `api/reviews`·`api/review_runtime`이 role_11 캘린더를 **직접**
쓴다. 리뷰 날짜 산술을 갈아끼우는 작업이고, 리뷰 경로는 같은 세션에 러너에서
막 떼어낸 참이라 두 번 흔들지 않았다. 대신 두 캘린더가 어긋나는 순간을 잡는
테스트를 뒀다(`tests/unit/test_calendar_agreement.py`).

**러너 삭제가 드러낸 것 — 문서가 틀렸던 지점.** §7과 dev-handoff의 동등성
패킷은 "role_11 리뷰는 러너와 무관하게 산다"고 적었다. 런타임에서는 맞지만
**데이터 층에서는 틀렸다**: `get_signal`이 `pipeline_runs`와 내부 조인을 해서
잡이 만든 판단 91건 중 0건만 매칭됐다. 러너를 지웠으면 리뷰 API가 영구히
아무것도 채점할 수 없게 될 뻔했다. 삭제 전에 수리했다(`e5379e1`).
