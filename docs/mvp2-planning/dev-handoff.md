...
1. **상장폐지된 보유는 팔 수 없다.** `tb_daily_pick → tb_universe` FK 때문에
   상장 피드에서 빠진 종목은 픽 행을 못 만들고 → 시그널을 못 남기고 → 청산이
   막힌다. 정확히 `delisting_halt`가 겨냥한 시나리오다. 지금은 스크리닝 잡이
   요약에 `skipped N unlisted holdings`로 남기기만 한다. FK 설계를 건드려야 해서
   임의 확대하지 않았다.
