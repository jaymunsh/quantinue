...
> 최종 갱신 2026-07-19 (Phase 3 분석 잡까지 반영). **이 파일은 "지금 어디까지 왔나"만 담는다.**
...
| 테스트 | 유닛/웹 **803 green** · 통합 **109 green** · ruff clean (재설계 착수 기준선은 681/63) |
| ⚠️ 통합 실행 | `-p no:unraisableexception` 필요. asyncpg 연결 GC의 `ResourceWarning`이 에러로 승격돼 15건이 가짜 실패로 뜬다 — 로직 실패가 아니다 |
| ⚠️ `.env` | 2026-07-19 문성혁이 Alpaca 키 재입력 완료(뉴스·봉 엔드포인트 실 200 확인). `.env.example`로 덮어쓰지 말 것 |
...
| 뉴스 일괄 수집 | ⏳ **다음 세션 착수점** — 소스는 확정(Alpaca 뉴스 API, 실 200 재확인), 소비자가 Phase 3 분석 잡이므로 같은 커밋에 |
...
| 성향 페르소나 2종 (프롬프트) | ⏳ 미착수 — 지금은 프로필이 **문턱만** 바꾸고 프롬프트에는 안 들어간다 |
| 뉴스 일괄 수집 | ⏳ 미착수 |
...
1. **상장폐지된 보유는 팔 수 없다** — ⚠️ **다음 세션 1순위**.
   `tb_daily_pick → tb_universe` FK 때문에 상장 피드에서 빠진 종목은 픽 행을
   못 만들고 → 시그널을 못 남기고 → 청산이 막힌다. 정확히 `delisting_halt`가
   겨냥한 시나리오다. 선택지 4개 비교와 권고안(A: 유니버스가 보유를 이월 +
   `listing_status` 컬럼), 착수 시 결정할 것 6개는 **`pipeline-redesign.md` §11**에
   정리했다. 방향은 문성혁 확인 완료(2026-07-19).
2. **제약 이름 드리프트 1건**(기존, 내 변경과 무관): `tb_order`의 close 타깃
   CHECK가 신규 설치 `tb_order_check1` / 마이그레이션 `tb_order_close_target_check`.
   정의는 동일하고 카탈로그 테스트는 컬럼 기준이라 통과한다.
3. **`ensure_holding_in_scope`를 지우지 않았다.** 문서(§4 각주)는 스크리닝 잡이
   ...
   폴백으로 남겼다.
4. **매도 경로는 실 실행에서 미발동.** 이 DB에 열린 포지션이 없고 mock 분석기가
   고정 고점수를 낸다. 유닛 6건이 덮지만 **실 포지션으로 한 번 돌려볼 것**.
