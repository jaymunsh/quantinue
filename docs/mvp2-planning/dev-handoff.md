...
> 최종 갱신 2026-07-20 (Phase 3 **완료** — 뉴스·soft path·유령 일괄까지 반영). **이 파일은 "지금 어디까지 왔나"만 담는다.**
...
| 테스트 | 유닛/웹 **874 green** · 통합 **120 green** · ruff clean (재설계 착수 기준선은 681/63) |
...
(22종목 × 성향 2 ≈ 15분). `max_tokens=256`이 `provider.py`에 리터럴로 박혀
있는데, 이유 문장이 길어지면 잘려서 구조화 출력 실패 → 재시도로 이어질 수 있다.
**다음 세션이 볼 가설이다** — 실패율과 함께 재보고, 맞으면 config로 옮길 것.
...
2. **매크로가 새 경로에 잡으로 없다.** `latest_macro`는 구 러너 role_04가 쓴
   행을 읽는다. 구 러너를 지우면(Phase 4) 매크로가 끊긴다 — 매크로 잡이 필요하다.
3. **dev DB에 심어둔 포지션 3건이 열려 있다**(TMO·NET·FTNT, 계좌
   `TEST-SELLGAP`, 진입가 = 종가×1.30, 체결일 2026-07-06). 매도 경로 측정용으로
   남겼다. 필요 없으면 청산 잡을 돌리면 시간 청산으로 닫힌다.
...
**Phase 3 완료(7/7) + 승인율 수리 완료.** 다음은 4(배분 잡·**구 러너 폐기 = 유일한
확인 지점**) → 5(정리).

Phase 4 착수 전 선결이던 "프롬프트에 지표가 없다"는 해소됐다(`2674fbe`+`2918fe2`,
승인 0/22 → aggressive 7/20 · conservative 3/20). 남은 것은 둘이다:
- **매크로 잡이 없다** — `latest_macro`가 읽는 행은 구 러너 role_04가 쓴다.
  구 러너를 지우면 `risk_off_action`·`macro_penalty_table`이 다시 유령이 된다.
- **`max_tokens=256` 리터럴 가설**(`provider.py`) — 이유 문장이 길어지면 잘려서
  구조화 출력 실패로 이어질 수 있다. 매 실행 2건 안팎이 model error로 빠진다.
  검증 안 한 가설이라 고치지 않았다.
