...
| 테스트 | 유닛/웹 **702 green** · 통합 **68 green** · ruff clean (Phase 1a·1b 반영. 재설계 착수 기준선은 681/63) |
...
| **1c 청산 잡** | ⏳ **다음** — 독립 잡, M1 스케줄러 탑재, 시간 청산(`exits.time_exit_bdays` 첫 소비) + 하드 이벤트 |
...
Phase 2(데이터층·시세 어댑터·시가평가) → 3(분석 잡·07 sell) → 4(배분 잡·**구 러너 폐기 = 유일한 확인 지점**) → 5(정리).
