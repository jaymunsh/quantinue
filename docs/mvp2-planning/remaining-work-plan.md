...
**실행 결과 (2026-07-20).**
- 일요일(뉴욕) 실시각 틱: 전 잡 `holiday` — 세션 없는 날 아무것도 안 한다.
- 월요일 시각 틱: 이미 돈 8종 `not_due` · disclosures 실 실행(1,738건 공시,
  하드 2) · daily_bars는 Alpaca 401로 실패 — **실패가 다른 잡을 끊지 않고
  사유가 원장에 남았다**(격리 설계의 실전 검증).
- enabled=true 기동: / 200, TERM 종료 0초.

**실행에서만 잡힌 결함 18 — 켜는 순간 앱이 안 죽는 상태였다.** lifespan의
task group이 run_forever를 기다린다. 구 코드는 LiveRunRuntime.cancel()이 전체
스코프를 취소해 줬는데 그 역할이 러너와 함께 조용히 사라졌다. enabled=false인
동안은 자식이 없어 드러나지 않았다. cancel_scope.cancel()로 수리.

**⚠️ 사용자 조치 필요 — Alpaca 키가 죽었다.** .env의 키로 bars·news 둘 다
401(curl 직접 확인 — 코드 문제 아님). 핸드오프의 "키 유효(실 200 재확인)"는
이제 사실이 아니다. Alpaca 대시보드에서 키 재발급 전까지 일봉·뉴스 수집이
매일 failed로 기록된다(다른 잡은 영향 없음).
...
**실행 결과 (2026-07-20).**
- 실물 확인: GNW는 **구조화 카테고리**(`<category domain=".../rss/stock">
  Nasdaq:DJT`)를 준다 — 산문 패턴보다 낫다(추출이 아니라 낭독). PRN은 산문
  관례 `(NYSE: PLNT)` — 미국 거래소 세그먼트만 읽고 못 찾으면 버린다.
  businesswire는 공개 RSS가 UA에 따라 차단되는 것을 실측해 **뺐다**(불안정한
  피드 하나가 잡을 매일 실패시키는 것보다 안정적인 둘로 시작).
- **별도 잡(`news_wire`)으로 등록** — Alpaca에 합성하지 않았다. 잡 격리가
  실패 경계라서다: 실제로 Alpaca 키가 죽은 지금도 와이어 수집은 돈다.
  무키 소스라 등록이 무조건이다(SEC와 같은 원리).
- article_id는 guid 해시(62비트) — RSS엔 정수 id가 없고 PK (article_id,
  ticker) 멱등이 성립해야 한다.
- `news_evidence`가 헤드라인에 출처를 싣는다(`[globenewswire.com] …`) —
  allow와 gray가 같은 목록에 섞이므로 모델이 무게를 달리 두려면 보여야 한다.
- 실 스모크(dev DB): news_wire 슬롯 succeeded · **29행/24티커** 적재 ·
  프롬프트 읽기에서 `[globenewswire.com] Trump Media Settles…` 확인.

- [x] 완료 (1단계 범위 — 2단계 news_score 채점 잡은 별도 결정)
