...
> 이 파일 자체도 세션 종료 시 갱신할 것. (최종 갱신 2026-07-20 후반 — **Phase 3 완료 + 승인율 수리**)
...
- Phase 3 전부: 성향 정합(2198467) · 일봉 백필(2c8f7b2) · 스크리닝 잡(724ddc2) ·
  ...
  승인율 수리(2674fbe: 지표를 프롬프트로 + 크리틱에 증거·범위 / 2918fe2: 07·08
  지시 충돌 해소) · LOCAL 재시도 config화 + 종목 단위 실패 격리(2674fbe)
...
--- 0. 승인율은 고쳤다 (2026-07-20 후반, 커밋 2674fbe + 2918fe2) ---

승인 0/22였던 것이 aggressive 7/20 · conservative 3/20이 됐다. 원인은 모델이
아니라 구조였고, 넷을 고쳤다: ① 스크리닝이 이미 계산한 창 지표를 프롬프트에
싣고(pick_indicators) ② 크리틱에게 같은 증거를 주고(critique_prompt) ③ 크리틱에게
우리 증거 범위를 알려주고 ④ "뒤집을 조건을 적어라"(07)와 "모순을 찾아라"(08)가
서로 싸우던 것을 풀었다. 상세는 dev-handoff "승인율 붕괴의 원인 추적".

**남은 가설 하나**: `provider.py`의 `max_tokens=256`이 리터럴로 박혀 있다.
이유 문장이 길어지면 잘려서 구조화 출력 실패 → 재시도로 이어질 수 있다(매 실행
2건 안팎이 model error로 건너뛴다). 실패율과 함께 재보고, 맞으면 config로 옮길 것.
...
- ~~LOCAL 경로 retries=0~~ 해소(2674fbe) — 실제로 터져서 conservative 22종목이
  통째로 날아갔다. 이제 llm_max_retries가 소유하고, 종목 단위로 격리되며,
  건너뛴 수가 잡 원장에 남는다. openai 전환 시 값만 재확인.
...
- 테스트만 믿지 말고 실제로 돌려볼 것. 지금까지 잡힌 결함 13건이 전부 테스트를
  통과한 뒤 실행에서 나왔다. 12번이 특히 볼 만하다:
...
검증:
  cd app-v2 && uv run pytest tests/unit tests/test_web.py -q   # 874 green 유지
  uv run ruff check src tests scripts
  통합(120 green)은 일회용 DB 전제 — 새 컨테이너에 db/schema.sql 적재 후 1회만.
...
실 스모크 방법:
  ...
  있고 모델은 Qwen3.6-35B-A3B-OptiQ-4bit다. 성향 2종 분석 한 바퀴에 약 6분씩
  (종목당 ~15초). 프롬프트가 길어져 이전 4분보다 늘었다.
