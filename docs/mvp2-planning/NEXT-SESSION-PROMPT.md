...
> 이 파일 자체도 세션 종료 시 갱신할 것. (최종 갱신 2026-07-19 — Phase 3 분석 잡까지 완료)
...
이어서 진행해라 — **Phase 3 잔여 4개**:

1. **성향 페르소나 2종 (프롬프트)** ← 여기부터
...
⚠️ 다음 세션이 판단할 것 (내가 남긴 미해결 4건 — dev-handoff에 상세):
- **상장폐지된 보유는 팔 수 없다.** tb_daily_pick → tb_universe FK 때문.
  delisting_halt가 겨냥한 바로 그 시나리오라 아이러니하다. 스크리닝 잡이
  "skipped N unlisted holdings"로 요약에만 남긴다. FK 설계 문제라 임의 확대 안 함.
