...
> 8020 owner만 5분마다 DB 5445 + 일일 잡 부착 + WatchRunner liveness를 확인해
> success 또는 `/fail`을 보낸다. 8021 web-only는 절대 보내지 않는다. URL은
> 로그·증거·문서에 출력하지 않는다. Healthchecks.io는 Period 5분 / Grace 5분,
> 기존 Telegram bot을 호출하는 Webhook Down+Up 연동이 운영 기준이다.
> 2026-07-23 success ping·외부 `Up`·Webhook 연결을 실측했다. bot token/chat ID와
> ping URL은 채팅·스크린샷에도 노출하지 않는다. 상세는 operations-runbook §2-1-1.

> ## 🔄 2026-07-22 20:40 KST — 운영 활성화 작업 진행 중 (아래 과거 상태보다 우선)
>
> 8020은 **guarded 단일 작업 소유자**, 8021은 web-only다. 현재 HEAD의 정본
> 후보 설정은 `watch=true`, `rejudge=false`, `stream=false`다. 다만 기존 8020이
> 이 커밋을 재로딩했다는 독립 증거는 없다. 1분 poll의 실제 실행 여부는 런타임
> 상태와 원장으로 다시 확인한다. 07-22 슬롯은 8021 경합으로
> 오염돼 `open-items.md` 3-3을 닫지 않았다. 다음 관문은 ① 07-22 22:30 KST
> 정규장 ready 2회 ② 07-23 13:00 KST clean 8020-only 슬롯 ③ rejudge
> ④ stream 순서다. 현재 명령은 `operations-runbook.md` §4-6~§4-7이 정본이다.
> 아래의 “기본 설정 꺼짐”, “8020 불가침”, “07-22 첫 슬롯 우선” 문구는 당시
> 기록이며 이 배너와 충돌하면 적용하지 않는다.
...
quantinue-integrated-design.html(설계 v6.3) · quantinue-engineering.html(부록 v1.3).
...
| W3-4 | ✅ 방어선 발동 내역(청산 사유를 관제실에) | 매도 체결 원장만 표시 |
| ⑦ | ✅ W2-4 벤치마크(SPY) 잡 → /me에 SPY 대비 | 274일 실 백필 확인 |
| ⑧ | ✅ 종결: 빈 필드 3종은 채우지 않음 | 크리틱 반박·성향 프롬프트와 중복, 소비자 없음 |
...
실 확인 환경:
  관측 앱 8020 · 코드 작업용 8021(mock) · DB 5445
  계정 자격 증명은 로컬 비밀 저장소에서만 확인하고 문서·로그에 기록하지 않는다.
