...
- 컨벤션 8규칙 계승 (BOOLEAN is_/has_ · ENUM=TEXT 소문자 · TIMESTAMPTZ · 점수 0~1 · JSON=컬럼명 · tb_/_signal/v_ · signal append 불변 · 대리키+UNIQUE(ticker, cycle_ts))
  - ✅ **규칙 4 개정 확정 (07-18)**: "NUMERIC(4,3) 예외 없음" → **"점수는 0~1, DB는 NUMERIC + CHECK(0~1), 표기·전달은 소수 3자리"**. 코드 현실(plain NUMERIC + CHECK) 수용 — 값 범위는 강제, 물리 정밀도(4,3) 강제는 해제(기존 데이터 라운딩 위험만 있고 실익 없음). v4.0 문서 컨벤션 섹션에 개정 표기.
- **투자 성향 ENUM = `aggressive`(공격형) · `conservative`(안전형)** 2종 확정 (07-18)
...
- **📖 문서 미러 규칙**: 각 마일스톤에서 **핵심 로직·프롬프트(페르소나)가 코드로 확정되면**, 그 확정본을 v4.x 설계서 `#logic` 섹션에 반영한다(설계값 `M2` → `as-built` 승격). 코드 먼저 확정 → 문서 반영 순서(추측 프롬프트 선기재 금지 — 문서-코드 괴리 방지). 특히 M4(게이트)·M5(매도·성향 페르소나)·M7(회고 lesson 프롬프트) 완료 시 필수, M9에서 전체 정합 최종 점검.
...
| 5 | 실행 **claim 락** `(component, cycle_ts)` + **catch-up**(기동/버튼 시 오늘 놓친 due 슬롯을 현재 데이터로 실행 — 과거 소급 없음) + 수동 트리거도 슬롯 함수 공유 (R9) | `orchestration/` claim 확장 · `POST /runs` |

**성공 기준**: ① 같은 슬롯에서 수동+자동 2회 실행 → signal 행 1개(E2E-2) ② 동시 발화 시 중복 실행 0(claim 락) ③ 휴장일에 07~10 미실행 ④ 서머타임 전환일 통과 ⑤ 재시작 후 밀린 슬롯 catch-up.
...
- **수동 컨트롤 (R9 · 장애 복구)**: 지금 사이클 실행 · 특정 역할만 실행 · 밀린 슬롯 따라잡기(catch-up) · 실패 단계 재실행(체크포인트 재개) · 리뷰 재채점 — 각 버튼은 대상 슬롯 표시 + 완료 슬롯이면 멱등 no-op 경고
...
### M11. 배포 (R2-5 · R9) — ⚠️ 최후·선택 사항

로컬 Docker Compose(app+Postgres) 완성이 **필수 완료선**. AWS 이전은 **선택**(상황상 못 할 수 있음) — 로컬 상시 구동(절전 방지 `caffeinate` + 개장 시간대 wake)으로 대체 가능. AWS 진행 시: 소형 인스턴스(t3.small급) · HTTPS · 시크릿 env/SSM · DB 백업(일 1회 dump) · 텔레그램 하트비트.

**성공 기준 (필수)**: **로컬에서 무인 하루 사이클 완주 + 관리자/유저 화면 동작** — 사람 개입 0회(수동 컨트롤은 복구용), E2E 시나리오 전부 재현. AWS 5영업일 연속 운영은 보너스 목표.
