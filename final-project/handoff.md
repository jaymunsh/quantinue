...
**영상·슬라이드·테스트가 전부 끝났다.** 요약본 `summary-3min.mp4` 2:55가
정본이고 §4-1의 5막·§4-2의 샷 A~J를 담고 있다. 덱은 본편 14화면 + 백업
2장이며 원고와 맞는다. 테스트는 803·190·Ruff 전부 통과(07-28 02:20).

**남은 일 정본은 `NEXT.md`다** — 알림 배포(오늘 13:00 마감), 본인 캡처
2장, 리허설, 덱 5화면 이후 검토.
...
| `slides/index.html` | 본편 14장 + 백업 2장 | 발표용 덱. `←/→` 이동 · `F` 전체화면 · **`P` 발표자 창** · `N` 노트(리허설용) |
| `slides/quantinue-final.pdf` | 16쪽 | 프로젝터 백업 |
