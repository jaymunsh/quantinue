...
- `watch-activity-panel`: ledger-backed intraday activity summary with latest KST timestamp, distinct ticker count, and persona-signal count. Its empty state explicitly says that absence of a non-midnight judgement does not prove whether the loop is alive; it never turns configuration into a liveness claim.
