...
로컬 우선 모의 트레이딩 애플리케이션이다. 일일 종목 발굴·분석 체인과 정규장
장중 감시를 분리하며, 외부 실주문 없이 `MockBroker` 주문·체결 원장을 운영한다.

## 바로 실행

일반 `uvicorn` 실행은 web-only가 기본이다. 운영 작업자는 반드시
`./scripts/run_observation.sh`로 시작하며, 이 실행기만 일일 JobRunner와 장중
WatchRunner 소유권을 켠다. 로컬 잠금이 두 번째 운영 작업자를 거부한다.
...
브라우저에서 `http://localhost:8000`을 열고 NVDA 파이프라인을 실행한다. 기본값은
...
호스트에서는 `127.0.0.1:5445`의 app-v2 전용 DB를 사용한다. 컨테이너 사이에서는
`db:5432`를 사용한다. 이 프로젝트는 호스트 `5432`를 게시하거나 기존 컨테이너를 조작하지
않는다. 최초 DB 볼륨 생성 시 [db/schema.sql](db/schema.sql)이 자동 적용된다.

운영 관측 인스턴스는 `./scripts/run_observation.sh`로만 시작한다. 이 명령만
백그라운드 잡과 장중 감시 소유권을 켜며, 로컬 단일-owner 잠금으로 중복 실행을
거부한다. 개발용 웹은 운영 매뉴얼의 8021 명령처럼 DB 5445, mock LLM,
`QUANTINUE_BACKGROUND_WORKERS=0`, `QUANTINUE_OPS_ALERTS=0`을 모두 명시한다.

운영 `.env`에 `QUANTINUE_HEARTBEAT_URL=https://hc-ping.com/<UUID>`를 두면
8020 owner가 Healthchecks.io에 5분마다 외부 생존 신호를 보낸다. DB 원장이
읽히고 일일 잡과 WatchRunner가 정상 부착된 경우만 성공이며, 저하는 `/fail`,
프로세스·맥·네트워크 종료는 ping 누락으로 드러난다. 이 URL은 인증정보이므로
로그·문서·커밋에 실제 값을 남기지 않는다. 8021 web-only는 보내지 않는다.
Healthchecks는 Period 5분 / Grace 5분으로 두고, 기존 Telegram bot의
`sendMessage` Webhook에서 Down/Up 알림을 보낸다. Webhook의 bot token과 chat ID도
비밀값이며 문서·로그·스크린샷에 남기지 않는다.
...
- 일일 체인: 유니버스·수집·스크리닝·두 persona 분석·청산·배분·요약을 뉴욕
  슬롯 기준으로 실행한다. 등록 14종의 상세 계약은 운영 매뉴얼 §4-6-1에 있다.
- 장중 polling: 정규장 중 1분마다 보유 종목과 당일 픽을 확인해 손절·익절을
  즉시 MockBroker 청산한다.
- 장중 rejudge: 사건(±5%)과 10:00/12:45/15:15 NY 스윕에서만 LLM을 호출한다.
- stream: 보유 종목 최대 30개만 Alpaca IEX 체결을 받고 polling은 fallback으로
  유지한다.

현재 HEAD의 정본 설정은 `watch=true`, `rejudge=false`, `stream=false`다.
이 값은 다음에 단일 작업 owner를 안전하게 재기동할 때 적용될 **커밋된 후보**다.
이미 실행 중인 8020이 이 값을 다시 읽었다는 뜻은 아니며, 실제 적용 여부는
관제실의 런타임 상태와 원장으로 별도 확인한다.

## 1차 MVP 수동 흐름 — 역사적 호환 경로
...
- 03: 결정론적 점수로 오늘의 후보 10개를 선정한다.
- 04~06: 거시경제·공시·종목별 뉴스 데이터를 수집하고 대표 근거를 분석한다.
...
| `role_03_daily_screener` | 2차 스크리너 | 결정론적 점수 기반 후보 10개 | 다계정 후보 정책 |
...
덮어쓴다.
1차 호환 경로의 `load_pipeline_document(...).due_role_scheduler().due_roles(...)`는
주어진 시각과 마지막 실행 시각으로 04·05·06·07 중 due 역할만 반환한다.
일반 web-only 프로세스에서는 worker를 시작하지 않는다. 운영 관측 실행기만
`mvp2.jobs`와 `mvp2.watch`를 백그라운드에서 소유한다.
